...
logs-for-%:
	...
	@echo "-----"
	-timeout -s9 $* docker logs -f ftpd_server

build:
	docker build --rm -t pure-ftp-demo .

run: kill
	docker run -d --name ftpd_server -p 21:21 -p 30000-30009:30000-30009 -e "PUBLICHOST=localhost" -e "ADDED_FLAGS=-d -d" pure-ftp-demo

# runs with auto generated tls cert & creates test bob user
run-tls: kill
	-docker volume rm ftp_tls
	docker volume create --name ftp_tls
	docker run -d --name ftpd_server -p 21:21 -p 30000-30009:30000-30009 -e "PUBLICHOST=localhost" -e "ADDED_FLAGS=-d -d --tls 2" -e "TLS_CN=localhost" -e "TLS_ORG=Demo" -e "TLS_C=UK" -e "TLS_USE_DSAPRAM=true" -e FTP_USER_NAME=bob -e FTP_USER_PASS=test -e FTP_USER_HOME=/home/ftpusers/bob -v ftp_tls:/etc/ssl/private/ pure-ftp-demo
...
test-bob-tls:
	echo "Test file was read successfully!" > test-orig-file.txt
	cert="$$(docker volume inspect --format '{{ .Mountpoint }}' ftp_tls)/pure-ftpd.pem";\
	echo "ls -alh\n\
	put test-orig-file.txt\n\
	echo '~ uploaded file ~'\n\
	ls -alh\n\
